...
import { asins, stats } from "./main.js";
...
const offer = async (e: Endpoint) => {
    ...
    for (let i = 0; i < sellers.length; i++) {
        ...
        product.dateHandled = new Date().toISOString();
        product.numberOfRetries = request.retryCount;
        product.currentPendingRequests = (await (await e.crawler.getRequestQueue()).getInfo())?.pendingRequestCount;
        products.push({ ...product });
    }
    products = filterDuplicateOffers(products);

    asins[request.userData.productDetail.asin] = products.length;
    stats.totalSaved += products.length;
